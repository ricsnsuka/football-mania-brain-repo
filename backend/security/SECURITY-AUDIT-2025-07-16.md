...
concerns (IDOR, privilege escalation, GROUP_ADMIN bypass, race condition, authentication
...
for modification — it is read only to enforce the GROUP_ADMIN block. Linking a player does not
...
- A token whose user has been promoted to GROUP_ADMIN since issuance will be blocked on the very
...
| `POST /api/players` | `@PreAuthorize("hasRole('ADMIN_USER') or hasRole('MASTER_USER')")` | GROUP_ADMIN or MASTER | ✅ |
| `PATCH /api/players/{id}` | `@PreAuthorize("hasRole('ADMIN_USER') or hasRole('MASTER_USER')")` | GROUP_ADMIN or MASTER | ✅ |
| `PATCH /api/players/{id}/status` | `@PreAuthorize("hasRole('ADMIN_USER') or hasRole('MASTER_USER')")` | GROUP_ADMIN or MASTER | ✅ |
| `DELETE /api/players/{id}` | `@PreAuthorize("hasRole('ADMIN_USER')")` | GROUP_ADMIN only | ✅ |
| `POST /api/players/{id}/link-me` | `@PreAuthorize("isAuthenticated()")` | Any auth user (GROUP_ADMIN blocked in service) | ✅ |
...
| 3 | GROUP_ADMIN Bypass | — | Authority check correct; loaded fresh from DB | ✅ PASS |
