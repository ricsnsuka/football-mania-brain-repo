...
Complete as of **V34**, verified against `src/main/resources/db/migration/` on 2026-08-04.
**Production is applied through V31.** V32, V33 and V34 are **not deployed**; V34 is not yet merged
either — it sits on the recurring-plans branch.
...
| V34 ⏳ | `V34__platform_settings.sql` | Adds `platform_settings` — the settings that belong to the **deployment** rather than to a group, the first being how far ahead a recurring match plan may be scheduled. **`app_settings` could not hold it.** V27 made that table's key `(tenant_id, setting_key)` and its endpoint is `GROUP_ADMIN`, so a limit stored there would have been raised by exactly the people it exists to bound — the same argument that keeps group creation behind an operator-issued code. **The one table in the schema with no `tenant_id`, deliberately**: there is no tenant to scope to, and a tenant predicate here would be the mistake rather than its absence. `RepositoryScopingGuardTest` does not flag it because `PlatformSettingValue` is not `TenantOwned`; access is governed by `PlatformGuard`. Shape is pre-V27 `app_settings` on purpose — a row exists only where an operator has overridden something, the enum default is the fallback, deleting a row is the reset. **Ships empty**, so it changes no behaviour on its own. **Not deployed, and never yet run against a real database.** |
