...
| V51 | `V51__ladder_season_awards.sql` | One `CHECK` widened on `season_awards.award` for the two rung awards, `HIGHEST_TIER` and `BIGGEST_CLIMB` (RANK-LADDER-PLAN §7 step 5). Dropped and recreated, as V38 found is the only way to widen a `CHECK`. Nothing backfilled: a season closed before it, or with the ladder off, gains no rung awards. Shipped 3.6.0 |
| V52 | `V52__group_creation_code_requests.sql` | **The bell on the locked door.** One table, `group_creation_code_requests`: who asked the operator for a creation code, an optional line, when, and what became of it (`PENDING` / `FULFILLED` / `DISMISSED`, with `resolved_at`, `resolved_by`, and `creation_code_id` → the ledger row issued in answer, `ON DELETE SET NULL`). A **partial unique index** on `(user_id) WHERE status = 'PENDING'` makes "one open ask per account" the database's rule as well as the service's — two taps on a slow connection are the normal way a duplicate arrives. Three `CHECK`s keep the states honest. `user_id` cascades: the message is the requester's personal data (⚠️ not yet in the export). Additive, **not a rollback boundary**. Contract: `CREATION-CODE-REQUESTS-API-CONTRACT.md` |
