...
| V51–V53 ⚠️ | `V51__ladder_season_awards.sql` · `V52__group_creation_code_requests.sql` · `V53__user_last_login_at.sql` | **Shipped (3.6.0, 3.8.0, 3.8.2), not yet described here** — found missing when V54 was added (2026-09-13). Each file opens with its own reasoning comment; read that until these rows are written |
| V54 | `V54__week_honours.sql` | **`week_honours`: the stamped record of each closed week's player and team of the week, per format.** One row per honour per player per format per week, unique on `(tenant_id, week_start, match_type, honour, player_id)`, composite FK to `players` per V25. Written by a daily sweep once the week has closed and every crowd MOTM poll on it has resolved, from `GET /api/honours/week`'s own answer; the endpoint itself still computes on read and reads nothing here. Exists for one reader: the two counted badges `PLAYER_OF_THE_WEEK` and `TEAM_OF_THE_WEEK`, whose `count` is the row count. Insert-only and permanent like a badge. **Nothing backfilled here** — a stamp taken today is today's answer, not what was announced then; `POST /api/admin/honours/week/backfill` is the deliberate, admin-triggered walk over the history. Additive, unread by the previous jar: **not a rollback boundary**. Personal data: row in [PRIVACY_AND_DATA_PROTECTION](../backend/features/PRIVACY_AND_DATA_PROTECTION.md) |
