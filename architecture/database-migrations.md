...
Complete as of **V39**, verified against `src/main/resources/db/migration/` on 2026-08-09.
...
⚠️ **V39 is written, not merged and not deployed, and has never run against real PostgreSQL.**
It carries `api_tokens` — the credential a shortcut, tablet or script holds instead of a JWT. The
session that wrote it had no Docker, so `integrationTest` never ran. That is now **three unverified
migrations stacked up** (V37, V38, V39), which is the one thing about this chain worth clearing
before the next deploy: run the integration tier somewhere with Docker and the whole stack is
confirmed at once.

⚠️ **V37 and V38 are written, not merged and not deployed.** They carry `season_awards` and the
Ballon d'Or poll — the only schema the seasons work has needed. The write API and the
season-bounded league table both went in without any.
