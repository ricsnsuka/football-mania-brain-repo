...
| Apple Watch shortcut guide | ✅ 1.9.1 | — | [guide](https://github.com/ricsnsuka/FootMania-Back/blob/main/docs/guides/watch-shortcut.md). Action-by-action instructions for whoever runs the match. Five menu options, not two — the four goal types are weighted differently, so a shortcut logging everything as a solo goal inflates ratings |
| Live stat deltas (no migration) | ✅ 1.9.0 | — | [contract](https://github.com/ricsnsuka/FootMania-Back/blob/main/docs/api/MATCH-LIVE-STATS-API-CONTRACT.md). `soloGoalsDelta` and its four siblings on `PATCH /api/matches/{id}/stats/live`, beside the absolute fields. **The other half of the watch shortcut** — 1.8.0 shipped the credential, this ships the request shape a caller with no screen can actually use. Delta requests take a row lock so two people recording the same passage of play queue rather than overwrite. Frontend unaffected: the fields are optional and additive |
...
| Creation code requests (V52) | ✅ built 2026-09-06, **not yet released** | ✅ built 2026-09-06, **not yet released** | [contract](https://github.com/ricsnsuka/FootMania-Back/blob/main/docs/api/CREATION-CODE-REQUESTS-API-CONTRACT.md) · [FE groups §asking](../frontend/features/groups.md#asking-for-a-creation-code--groupsnew-2026-09-06) · [FE settings](../frontend/features/settings.md). A prospective founder asks for a code from `/groups/new`; every operator gets a push and an alert on `/platform`; fulfilling issues a code through the ledger and fills it into the requester's form. Same change: notification toggles are per audience (an operator sees one), the Settings Platform tab is gone (the console is reached from the account menu), and the Draft Sessions nav entry is gone (it is a Team Generation tab) |
