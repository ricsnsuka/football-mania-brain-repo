...
| Phase 5 — multi-tenancy + billing | 🟡 **Two rungs built.** [schema](../backend/plans/TENANCY-SCHEMA-PLAN.md) ✅ 2026-08-01 (V22–V27) → [enforcement](../backend/plans/TENANCY-ENFORCEMENT-PLAN.md) ✅ 2026-08-02 (`X-Group-Id`, predicates, async payloads, SSE ownership, V28 platform grant) → [privacy](../backend/plans/TENANT-PRIVACY-PLAN.md) 📝 → [onboarding](../backend/plans/GROUP-ONBOARDING-PLAN.md) 📝 **the visibility flip, owner-gated** → [billing](../backend/plans/GROUP-BILLING-PLAN.md) ⛔ **ON HOLD (owner)**. Both built rungs ship **dark and are not deployed** — production is on V21. Owner decisions locked: multi-membership, picker addressing, code-gated creation, tenancy before billing |
