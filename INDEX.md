...
| [RANK-LADDER-PLAN](backend/plans/RANK-LADDER-PLAN.md) 🟡 | **Steps 1, calibration round 1 and step 2 in `next`, not released** ([Back#278](https://github.com/ricsnsuka/FootMania-Back/pull/278)): the engine, V50, the read surface behind a per-group switch, and start-finalises; steps 3–5 unbuilt — Iron→Master tiers of three 75-FP divisions in front of the raw rating, **Form Points** paid per match from result and performance, three placement matches per season, a soft reset that reads FP, and **Best of the Crop** for the top Masters. Owner decisions taken 2026-09-04; every other constant waits for a history replay (§8). Target `3.5.0` |
