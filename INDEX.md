...
| [PROVISIONAL-TEAMS-PLAN](backend/plans/PROVISIONAL-TEAMS-PLAN.md) 🟡 | IN PROGRESS — epic #92. All five PRs are merged into `next` (FootMania-Back#358–#360, FootMania-Simple-Front#239–#240) and none is released; backend 3.14.0 deploys first. Left: the releases, then C and retiring `/generate/confirm`. Decided 2026-09-24: team generation saves provisional pairings on the plan instead of creating the match; they are flagged when the starters change, regenerated on the same model, and turned into a match when a manager picks one. Same epic: charges from the team sheet (reserves no longer pay), a system message and a push announcing the teams, and a reserve cutoff before kickoff. §7 lists the traps (chat has no system sender, the chat push would fire twice) |
...
| [RANK-LADDER-PLAN](backend/plans/RANK-LADDER-PLAN.md) ✅ | **Shipped dark in 3.5.0, 2026-09-04** — the engine, V50, the read surface behind a per-group switch, start-finalises, the rung chip and ladder chart; per group still to do: backfill, calibration read, switch on (group 1: backfilled and read 2026-09-04, switch still off). Step 5 (rung badges, two season awards) shipped dark in 3.6.0, 2026-09-05 — Iron→Master tiers of three 75-FP divisions in front of the raw rating, **Form Points** paid per match from result and performance, three placement matches per season, a soft reset that reads FP, and **Best of the Crop** for the top Masters. Owner decisions taken 2026-09-04; every other constant waits for a history replay (§8). Shipped `3.5.0` + `3.6.0` |
...
| [AGENTS.md](https://github.com/ricsnsuka/FootMania-Back/blob/main/AGENTS.md) · [`.claude/`](https://github.com/ricsnsuka/FootMania-Back/tree/main/.claude) — how an agent behaves there: rules, hooks, `/release`, `/verify` | backend root |
...
| [AGENTS.md](https://github.com/ricsnsuka/FootMania-Simple-Front/blob/main/AGENTS.md) · [`.claude/`](https://github.com/ricsnsuka/FootMania-Simple-Front/tree/main/.claude) — *"this is NOT the Next.js you know"*, and how an agent behaves there | frontend root |
