...
release number and `/api/version` are the two rows to fill in, and **neither `v3.8.2` tag is placed** — the backend's waits for those rows, the frontend's
for a terminal that may push tags. The [3.8.2 section](#382--shipped-2026-09-10-early-afternoon-backend-readback-pending)
