...
stamped at login). Frontend read back from Netlify (deploy `6aa2a272`, `445475b` = `main`); backend read back by
the owner on 2026-09-11 (`/api/version` `3.8.2`, `/api/health` `UP`), the Heroku release number
still to fill in. **Neither `v3.8.2` tag is on a remote yet**: both exist locally with their
evidence, and the session's git proxy refuses tag pushes. The [3.8.2 section](#382--shipped-2026-09-10-early-afternoon-confirmed-2026-09-11-tags-to-place)
...
## 3.8.2 — shipped 2026-09-10, early afternoon; confirmed 2026-09-11, tags to place
...
| Running in production | **`3.8.2`, read back by the owner from a terminal on 2026-09-11**: `/api/version` reports `3.8.2` and `/api/health` is `UP` — which doubles as the V53-applied evidence, since Flyway runs on boot. Heroku builds `main` on push, so the running commit is `adf7b12`. ⚠️ **Heroku's own release number was not read back**: the session that cut this release ran in a container whose egress proxy refuses every Heroku host and holds no Heroku login. Fill it in from `heroku releases -a footmania` (the "Deploy adf7b121" row) | **`445475b`, confirmed by asking Netlify**: deploy `6aa2a272bd19220008c7ba3b`, `ready`, `production`, branch `main`, `commit_ref` `445475ba18f023a19ae914a3e9ced2df69ed0637` — equal to `main` — published `2026-09-10T12:29:32.339Z`, 55s build. Merged before the backend was confirmed, deliberately: the change is three locale strings and assumes nothing of the backend, which is the tolerance [CONTRIBUTING](CONTRIBUTING.md#deployment-order) asks for when the order cannot hold |
...
| Tags | ⚠️ **`v3.8.2` created locally at `adf7b12`, not on the remote** — annotated with the readback above, but the session's git proxy answers `403` to any tag push and no GitHub tool it holds can create a tag. Place it from a terminal: `git tag -a v3.8.2 adf7b12` with that evidence, then `git push origin v3.8.2` | ⚠️ **`v3.8.2` not yet placed either** — created locally at `445475b` with the Netlify evidence in its annotation, but the session's git proxy answers `403` to any tag push. Place it from a terminal: `git tag -a v3.8.2 445475b` annotated with deploy `6aa2a272bd19220008c7ba3b`, then `git push origin v3.8.2` |
...
**The button layout 3.1.0 shipped unchecked, finished properly — and checked this time.** The
owner caught two things live: the guest row's ⋯ menu broke the Confirm/Decline columns (a
reserved-spacer fix was tried and superseded), and the player modal's admin actions sat scattered
across five shelves with alternating alignments. The owner's call put the ⋯ **beside the guest's
name** (`FootMania-Simple-Front#122`), making every row's actions block identical — the alignment
question ends structurally — and the modal tour gained a step teaching guest removal, since the ⋯
is where it lives and nothing else says so. The player modal's routine controls (status, details,
link/unlink) now share one neutral left-aligned row; colour arrives only with consequence, in the
confirm panels. Verified on a local dev pair against the real backend before release — the check
3.1.0 skipped. Backend untouched, like 2.3.1: no backend CHANGELOG entry, backend tag stays
`v3.1.0`. Netlify deploy `6a95757a` read back from the API, `commit_ref` = `e6f3a83`.
